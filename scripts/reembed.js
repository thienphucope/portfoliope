...
if (!MONGODB_URI || !GEMINI_API_KEY) {
  console.error("❌ Thiếu MONGODB_URI hoặc GEMINI_API_KEY trong .env");
  process.exit(1);
}
...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < docs.length; i++) {
      const doc = docs[i];
      const content = doc.text || doc.content || ""; // Tùy chỉnh field text của bạn
      ...
      // Tránh bị rate limit API (nghỉ 200ms mỗi lượt)
