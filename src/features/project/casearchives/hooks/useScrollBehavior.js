...
export function useScrollBehavior({ appShellRef, tabs, setShowHeader, setShowFunctionBall }) {
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      ...
      const vScrollable = e.target.closest('.markdown-container, .file-list, .horizontal-tabs-container, .search-results, .note-gallery-container');
      
      // Special case: Allow these to scroll natively without custom animation or horizontal hijacking
      if (e.target.closest('.pdf-body, .chat-history, canvas')) {
        return; 
      }
