  ...
  useEffect(() => {
    const initPlayers = () => {
      if (window.YT && window.YT.Player && !bgPlayerRef.current) {
        bgPlayerRef.current = new window.YT.Player('bg-player-home', {
          videoId: '305Uc8i5RJM',
          playerVars: {
  ...
  return (
    <>
      ...
      <div className="w-full relative min-h-[100dvh]">
        ...
        {fingerprints.map(fp => (
          <div key={fp.id} className="fingerprint-effect" style={{ left: fp.x, top: fp.y, '--rot': `${fp.rotation}deg`, color: isIntroStarted ? 'var(--colorone)' : '#F5F5DC' }}>
        ...
        {!isIntroStarted && (
          <div className="fixed inset-0 bg-[var(--colorone)] flex flex-col justify-center items-center z-[100]">
            ...
            <div className="text-black font-fredericka text-xs md:text-2xl opacity-90 mt-4">opewatson.org {"->"} opewatson.com</div>
          </div>
        )}

        <header className={`absolute top-0 left-0 right-0 p-4 md:p-10 flex justify-between items-center z-[60] transition-opacity duration-1000 ${isIntroStarted ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <footer className={`absolute bottom-0 left-0 right-0 p-4 pb-10 md:p-10 flex flex-col lg:flex-row justify-center lg:justify-between items-center gap-4 lg:gap-0 z-[60] bg-gradient-to-t from-black/80 to-transparent transition-opacity duration-1000 ${isIntroStarted && !isStoryOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <div className="order-1 lg:order-1 text-[var(--colorone)] font-fredericka text-xs md:text-xl opacity-60">© 2026 Ope Watson</div>
