...
<div ref={scrollRef} className="w-full min-h-screen flex flex-col relative">
        ...
        <header className={`fixed top-0 left-0 right-0 p-4 md:p-10 flex justify-between items-center z-[60] transition-opacity duration-1000 ${isIntroStarted ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="flex items-center gap-4 md:gap-10 font-fredericka">
            <CaseArchivesCanvas />
            <Link href="/about" className="text-xl md:text-3xl font-semibold text-[var(--colorone)] hover:text-white cursor-pointer transition-colors">
              About
            </Link>
          </div>
        </header>
        ...
        <footer className={`fixed bottom-0 left-0 right-0 p-4 pb-2 md:p-10 flex flex-col lg:flex-row justify-center lg:justify-between items-center gap-4 lg:gap-0 z-[60] bg-gradient-to-t from-black/80 to-transparent transition-opacity duration-1000 ${isIntroStarted && !isStoryOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <Link href="/privacy" className="order-1 lg:order-1 text-[var(--colorone)] font-fredericka text-xs md:text-xl opacity-60 hover:opacity-100 transition-opacity">
            Privacy
          </Link>
