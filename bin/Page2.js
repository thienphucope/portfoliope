...
// --- COMPONENT CÂY THƯ MỤC (SỬA NHẸ) ---
// Sửa item.handle thành item.path
...
export default function RedMathVault() {
  ...
  // Registry để map tên file -> đường dẫn (cho tính năng Internal Link [[Link]])
  const fileRegistry = useRef({}); 

  // --- LOGIC MỚI: Tự động fetch file tree từ API ---
  useEffect(() => {
    const fetchTree = async () => {
      try {
        ...
        // Xây dựng registry phẳng để tra cứu nhanh cho internal links
        ...
        setContent("# Error\nCould not load cases folder. Please ensure `/public/cases` exists.");
      }
    };
  ...
  const loadFile = async (path, name) => {
    try {
      ...
      // Xử lý Internal Links [[Wiki Style]]
  ...
  return (
    <div className="app-container">
      ...
      <style jsx global>{`
        ...
        :root {
          --bg-color: var(--background); /* Hoặc #f4f4f4 nếu test riêng */
          --text-red: #8a0000;
          --bright-red: #ff0000;
          --sidebar-width: 300px;
        }
        ...
        .sidebar-header { padding: 30px 20px; text-align: center; margin-bottom: 10px; }
        ...
        .tree-item {
          ...
          color: var(--text-red); opacity: 0.8; transition: opacity 0.2s;
        }
        ...
        /* --- MAIN VIEWPORT (ĐẨY QUA PHẢI) --- */
        .main-viewport {
          width: 100%;
          padding-left: var(--sidebar-width); /* Chừa chỗ cho sidebar */
        ...
        .page-title { 
          ...
          margin-bottom: 40px; color: var(--text-red);
          text-align: center;
        }
        ...
        .markdown-body {
          ...
          font-size: 20px; line-height: 1.6; width: 100%;
        }
        ...
        .markdown-body th { font-weight: 400; border-bottom: 2px solid var(--text-red) !important; }
        ...
        .markdown-body h1, .markdown-body h2 { border: none !important; color: var(--text-red) !important; padding-bottom: 0.3em; margin-bottom: 1em; font-weight: 400; }
